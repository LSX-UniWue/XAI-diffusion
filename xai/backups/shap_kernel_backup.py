...
class Kernel(Explainer):
    ...
    def __init__(self, model, data, link=IdentityLink(), **kwargs):
        ...
        # NEWCODE add dynamic background kwarg parameter
        self.dynamic_background = kwargs.get("dynamic_background", None)  # 'optimized'
        self.full_model = kwargs.get("full_model", None)
        self.diffusion_model = kwargs.get("diffusion_model", None)
    ...
    def explain(self, incoming_instance, **kwargs):
        ...
        # NEWCODE
        if self.dynamic_background == 'diffusion':
            self.varyingInds = np.array(range(self.data.data.shape[1]))
        else:
        ...
        # if more than one feature varies then we have to do real work
            ...
            # NEWCODE
            # use SHAP's maskMatrix and replace the synth_data with the ones generated by Takeishi's method
            if self.dynamic_background:
                from xai.automated_background_torch import dynamic_synth_data
