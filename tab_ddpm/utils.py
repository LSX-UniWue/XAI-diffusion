...
def index_to_log_onehot(x, num_classes):
    x_onehot = index_to_onehot(x, num_classes)
...
def log_sub_exp(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    m = torch.maximum(a, b)
    log_res = torch.log(torch.exp(a - m) - torch.exp(b - m))
    log_res[log_res == -float('inf')] = 0.0
    return log_res + m
...
def sliced_logsumexp(x, slices):
    ...
    slice_lse = log_sub_exp(lse[:, slice_ends.long()], lse[:, slice_starts.long()])
